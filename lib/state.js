...
for (const [k, v] of Object.entries(store.get('modes') || {})) if (PERMISSION_MODES.includes(v)) sessionModes.set(k, v);
function persistModes() { store.set('modes', Object.fromEntries(sessionModes)); }
...
for (const id of (store.get('archived') || [])) if (typeof id === 'string') archivedSessions.add(id);
function setArchived(sessionId, archived) {
  ...
  store.set('archived', [...archivedSessions]);
}
...
for (const [k, v] of Object.entries(store.get('models') || {})) if (typeof v === 'string' && v !== 'default' && isValidModel(v)) sessionModels.set(k, v);
function persistModels() { store.set('models', Object.fromEntries(sessionModels)); }
...
module.exports = { SKIP_PERMS, PERMISSION_MODES, sessionModes, persistModes, archivedSessions, setArchived, setSessionMode, isValidModel, sessionModels, persistModels, setSessionModel, effectiveModel, effectiveMode };
